...
struct segTree{
    ...
    int first_above(int x, int l, int node, int lx, int rx){
        ...
        int m = (lx + rx) / 2;
        int res = first_above(x, l, 2 * node + 1, lx, m);
        if(res == -1){
            res = first_above(x, l, 2 * node + 2, m + 1, rx);
        }
        return res;
    }

    int first_above(int x, int l){
        return first_above(x, l, 0, 0, arraySize);
    }
